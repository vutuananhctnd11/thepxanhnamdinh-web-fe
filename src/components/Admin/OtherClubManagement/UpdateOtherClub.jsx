...
const UpdateOtherClub = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    //upload file
    try {
      let combinedData = { ...values, clubId };
      if (logoFile) {
      ...
      if (response.status === "success") {
        messageApi.success({
          content: "Cập nhật thông tin CLB thành công!",
          duration: 3,
        });
        setClub(response.data);
      }
...
export default UpdateOtherClub;
